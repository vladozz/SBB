...
var $chForward = $("<input/>").attr('class', 'form-control').attr('type', 'checkbox').attr('id', 'inputForward');
...
function addTrip() {
    var pathId = $('#pathSelect').find("option:selected").val();
    var trainId = $('#trainSelect').find("option:selected").val();
    var forward = $('#forward').prop('checked');
    if (!checkFields(trainId, pathId)) {
        return;
    }
    $.post('/SBB/trip/add',
        { pathId: pathId, trainId: trainId, forward: forward},
        function (response) {
            if (isError(response)) {
                findTrips();
                return;
...
function createRow(trip) {
    ...
    $.each(trip, function (key, value) {
        var $tdd = $td.clone().attr('class', key).text(value);
        if (key === 'forward') {
            $tdd.attr('class', key + ' hidden');
        }
        row.append($tdd);
    });
...
function initEditForm($content, tripId) {
    ...
    $content.append($fg.clone().append($tmpLbl).append($tmpSel));
    var forward = $('#' + tripId + ' .forward').text();
    $tmpLbl = createLabel('inputForward', 'Forward');
    $tmpSel = createCheckBox(forward).removeAttr('class');
    $content.append($fg.clone().append($tmpLbl).append($tmpSel));
}
...
function createCheckBox(forward) {
    return $chForward.clone().prop('checked', forward === 'true');
}
